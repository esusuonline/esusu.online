                ...
                <div class="table-responsive--md  table-responsive">
                    <table class="table table--light style--two">
                        ...
                        <tr>
                            ...
                            @php
                                $agent = DB::table('staffs')->where('id', $loan->user->agent_id)->first();
                            @endphp
                            <td data-label="@lang('Username')">
                                <a href="{{ route('admin.users.detail',$loan->user->id) }}">{{ __($loan->user->username) }}</a>
                                <br>
                                <span>Agent: <a href="{{ route('admin.users.detail',$loan->user->id) }}">{{ $agent->username }}</a></span>
                            </td>
                            <td data-label="@lang('Loan Plan')">{{ __($loan->name) }}</td>
                            ...
                            <td data-label="@lang('Action')">
                                <div class="d-flex flex-wrap justify-content-end button-wrapper">
                                    <button class="icon-btn btn--primary" data-toggle="tooltip" title="" data-original-title="@lang('View Savings History')">
                                        <a href="{{ url('fsp/loan/view/savings/history/'.$loan->user->id.'/'.$loan->loan_plan_id) }}"><i class="las la-eye text-white"></i></a>
                                    </button>
                                    @if ($loan->status == 0)
                                        <a href="{{ route('fsp.loan.pending.details', $loan->id) }}"  class="icon-btn" data-toggle="tooltip" title="" data-original-title="@lang('Details')">
                                            <i class="las la-desktop"></i>
                                        </a>
                                    @else
                                        <button class="icon-btn details-btn" data-toggle="tooltip" title="" data-original-title="@lang('Details')"
                                            data-plan_name = "{{ __($loan->name) }}"
                                            data-interval="{{ $loan->loan_type }} @lang(' Payable')"
                                            ...
                                            data-last_installment="{{ $loan->last_pay ? showDateTime($loan->last_installment, 'M-d, y') : '-' }}"
                                            data-next_installment="{{ $loan->next_pay ? showDateTime($loan->next_installment, 'M-d, y') : '-' }}"
                                            >
                                            <i class="las la-desktop"></i>
                                        </button>
                                    @endif
...
<div id="installmentModal" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('staff.user.loan.installment')}}" method="POST">
                @csrf
