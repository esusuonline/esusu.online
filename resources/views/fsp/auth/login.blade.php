    ...
    <div class="page-wrapper default-version">
        <div class="form-area bg_img" data-background="{{asset('assets/admin/images/1.jpg')}}">
            <div class="form-wrapper">
                ...
                <form action="{{ route('fsp.login') }}" method="POST" onsubmit="return submitUserForm();" class="account--form cmn-form mt-30">
                    @csrf
                    ...
                    <div class="form-group d-flex justify-content-between align-items-center">
                        <a href="{{ route('fsp.password.request') }}" class="text-muted text--small"><i class="las la-lock"></i>@lang('Forgot password?')</a>
                    </div>
                    <div class="form-group">
                        <button type="submit" class="submit-btn mt-25">@lang('Login') <i class="las la-sign-in-alt"></i></button>
                    </div>
                    <div class="text-center">
                        <span class="text-dark"> @lang('Don\'t have an account?') </span> <a href="{{ route('fsp.register') }}">@lang('Register now.')</a>
                    </div>
                </form>
            </div>
        </div><!-- login-area end -->
