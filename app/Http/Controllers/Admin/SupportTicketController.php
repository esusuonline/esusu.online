...
class SupportTicketController extends Controller
{
    ...
    public function staffTickets()
    {
        $pageTitle = 'Support Tickets';
        $emptyMessage = 'No Data found.';
        $items = SupportTicket::orderBy('id','desc')->where('staff_id', '!=', 0)->with('staff')->paginate(getPaginate());
        return view('admin.support.staff_tickets', compact('items', 'pageTitle','emptyMessage'));
    }

    public function staffTendingTicket()
    {
        $pageTitle = 'Pending Tickets';
        $emptyMessage = 'No Data found.';
        $items = SupportTicket::whereIn('status', [0,2])->orderBy('priority', 'DESC')->orderBy('id','desc')->where('staff_id', '!=', 0)->with('staff')->paginate(getPaginate());
        return view('admin.support.staff_tickets', compact('items', 'pageTitle','emptyMessage'));
    }

    public function staffClosedTicket()
    {
        $emptyMessage = 'No Data found.';
        $pageTitle = 'Closed Tickets';
        $items = SupportTicket::where('status',3)->orderBy('id','desc')->where('staff_id', '!=', 0)->with('staff')->paginate(getPaginate());
        return view('admin.support.staff_tickets', compact('items', 'pageTitle','emptyMessage'));
    }

    public function staffAnsweredTicket()
    {
        $pageTitle = 'Answered Tickets';
        $emptyMessage = 'No Data found.';
        $items = SupportTicket::orderBy('id','desc')->where('staff_id', '!=', 0)->with('staff')->where('status',1)->paginate(getPaginate());
        return view('admin.support.staff_tickets', compact('items', 'pageTitle','emptyMessage'));
    }


    public function staffTicketReply($id)
    {
        $ticket = SupportTicket::with('staff')->where('id', $id)->firstOrFail();
        $pageTitle = 'Reply Ticket';
        $messages = SupportMessage::with('ticket')->where('supportticket_id', $ticket->id)->orderBy('id','desc')->get();
        return view('admin.support.staff_reply', compact('ticket', 'messages', 'pageTitle'));
    }
    public function staffTicketReplySend(Request $request, $id)
    {
        $ticket = SupportTicket::with('staff')->where('id', $id)->firstOrFail();
        $message = new SupportMessage();
        if ($request->replayTicket == 1) {
    ...
    public function staffTicketDownload($ticket_id)
    {
        $attachment = SupportAttachment::findOrFail(decrypt($ticket_id));
        $file = $attachment->attachment;
    ...
    public function staffTicketDelete(Request $request)
    {
        $message = SupportMessage::findOrFail($request->message_id);
        $path = imagePath()['ticket']['path'];
        if ($message->attachments()->count() > 0) {
