...
class TicketController extends Controller
{
    ...
    public function supportTicket()
    {
        ...
        $supports = SupportTicket::where('fsp_id', Auth::guard('fsp')->id())->orderBy('priority', 'desc')->orderBy('id','desc')->paginate(getPaginate());
        return view('fsp.support.index', compact('supports', 'pageTitle', 'emptyMessage'));
    }
    ...
    public function openSupportTicket()
    {
        ...
        return view('fsp.support.create', compact('pageTitle', 'fsp'));
    }
    ...
    public function storeSupportTicket(Request $request)
    {
        ...
        $fsp = auth()->guard('fsp')->user();
        $ticket->fsp_id = $fsp->id;
        $random = rand(100000, 999999);
        $ticket->ticket = $random;
        $ticket->name = $request->firstname.' '.$fsp->lastname;
        $ticket->email = $fsp->email;
        ...
        $adminNotification->click_url = urlPath('admin.fsp.ticket.view',$ticket->id);
        ...
        return redirect()->route('fsp.ticket')->withNotify($notify);
    }
    ...
    public function viewTicket($ticket)
    {
        ...
        $my_ticket = SupportTicket::where('ticket', $ticket)->where('fsp_id',Auth::guard('fsp')->id())->orderBy('id','desc')->firstOrFail();
        $messages = SupportMessage::where('supportticket_id', $my_ticket->id)->orderBy('id','desc')->get();

        return view('fsp.support.view', compact('my_ticket', 'messages', 'pageTitle'));

    }

    public function replyTicket(Request $request, $id)
    {
        $ticket = SupportTicket::where('fsp_id', Auth::guard('fsp')->id())->where('id',$id)->firstOrFail();
        $message = new SupportMessage();
